<div class="section">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['clean_action'])) {
        ...
        try {
            ...
            if ($cleanAction === 'domain_query_logs' || $cleanAction === 'all') {
                $stmt = $conn->prepare("DELETE FROM domain_query_logs WHERE query_time < ?");
                $stmt->execute([$cutoffDate]);
                $cleaned['domain_query_logs'] = $stmt->rowCount();
            }
    ...
    $stmt = $conn->query("SELECT COUNT(*) as cnt FROM domain_query_logs");
    $stats['domain_query_logs'] = $stmt->fetch()['cnt'];
    ...
    <div class="card" style="background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 1.5rem;">
        ...
        <div style="display: flex; gap: 0.5rem; margin-bottom: 1rem;">
            ...
            <button type="button" onclick="switchLogTab('domain')" id="btn-log-domain" style="padding: 0.5rem 1rem; border: none; border-radius: 6px; cursor: pointer; background: #e9ecef; color: #333; font-size: 0.875rem;">🌐 域名查询日志</button>
        </div>
        ...
        <!-- 域名查询日志 -->
        <div id="log-panel-domain" style="display: none;">
            <?php
            $stmt = $conn->query("SELECT d.*, w.name as website_name FROM domain_query_logs d LEFT JOIN websites w ON d.website_id = w.id ORDER BY d.query_time DESC LIMIT 50");
            $domainLogs = $stmt->fetchAll();
            ?>
            <div style="overflow-x: auto; max-height: 400px; overflow-y: auto;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem;">
                    <thead style="position: sticky; top: 0; background: #f8f9fa;">
                        <tr>
                            <th style="padding: 0.5rem; text-align: left; border-bottom: 1px solid #ddd;">查询时间</th>
                            <th style="padding: 0.5rem; text-align: left; border-bottom: 1px solid #ddd;">域名</th>
                            <th style="padding: 0.5rem; text-align: left; border-bottom: 1px solid #ddd;">查询接口</th>
                            <th style="padding: 0.5rem; text-align: center; border-bottom: 1px solid #ddd;">状态</th>
                            <th style="padding: 0.5rem; text-align: center; border-bottom: 1px solid #ddd;">到期天数</th>
                            <th style="padding: 0.5rem; text-align: left; border-bottom: 1px solid #ddd;">详情</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($domainLogs as $log): ?>
                        <tr>
                            <td style="padding: 0.5rem; border-bottom: 1px solid #eee; white-space: nowrap; color: #666;"><?php echo date('m-d H:i', strtotime($log['query_time'])); ?></td>
                            <td style="padding: 0.5rem; border-bottom: 1px solid #eee;"><?php echo htmlspecialchars($log['domain']); ?></td>
                            <td style="padding: 0.5rem; border-bottom: 1px solid #eee;">
                                <span style="background: #e8eaf6; color: #3949ab; padding: 0.15rem 0.4rem; border-radius: 3px; font-size: 0.75rem;"><?php echo htmlspecialchars($log['api_used']); ?></span>
                            </td>
                            <td style="padding: 0.5rem; border-bottom: 1px solid #eee; text-align: center;">
                                <?php if ($log['success']): ?>
                                    <span style="color: #28a745;">✓ 成功</span>
                            ...
                            <td style="padding: 0.5rem; border-bottom: 1px solid #eee; text-align: center;">
                                <?php if ($log['days_remaining'] !== null): ?>
                                    <?php if ($log['days_remaining'] > 30): ?>
                                        <span style="color: #28a745;"><?php echo $log['days_remaining']; ?>天</span>
                                    <?php elseif ($log['days_remaining'] > 7): ?>
                                        <span style="color: #ffc107;"><?php echo $log['days_remaining']; ?>天</span>
                                    <?php else: ?>
                                        <span style="color: #dc3545;"><?php echo $log['days_remaining']; ?>天</span>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <span style="color: #999;">-</span>
                                <?php endif; ?>
                            </td>
                            <td style="padding: 0.5rem; border-bottom: 1px solid #eee; font-size: 0.8rem; color: #666; max-width: 250px; word-break: break-all;">
                                <?php
                                $detailParts = [];
                                if ($log['expiry_date']) {
                                    $detailParts[] = '到期: ' . $log['expiry_date'];
                                }
                                if ($log['error_message']) {
                                    $detailParts[] = '错误: ' . htmlspecialchars($log['error_message']);
                                }
                                // 解析raw_response显示查询耗时
                                if ($log['raw_response']) {
                                    $rawData = json_decode($log['raw_response'], true);
                                    if ($rawData && isset($rawData['duration_ms'])) {
                                        $detailParts[] = '耗时: ' . $rawData['duration_ms'] . 'ms';
                                    }
                                }
                                echo htmlspecialchars(implode(' | ', $detailParts) ?: '-');
                                ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                ...
                <?php if (empty($domainLogs)): ?>
                    <p style="text-align: center; color: #999; padding: 2rem;">暂无域名查询日志</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    function switchLogTab(type) {
        ...
        document.getElementById('log-panel-domain').style.display = 'none';
        ...
        document.getElementById('btn-log-domain').style.background = '#e9ecef';
        document.getElementById('btn-log-domain').style.color = '#333';
    ...
    <div class="card" style="background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 1.5rem;">
        ...
        <div style="margin-bottom: 1.5rem;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    <td style="padding: 0.75rem; border-bottom: 1px solid #eee;">🌐 域名查询日志</td>
                    <td style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #eee;"><?php echo $stats['domain_query_logs']; ?> 条</td>
                    <td style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #eee;"><?php echo $settings['history_retention_days'] ?? 30; ?>天</td>
                </tr>
            </table>
        </div>
        ...
        <form method="POST" onsubmit="return confirm('确定要清理数据吗？此操作不可恢复！');">
            ...
            <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                ...
                <button type="submit" name="clean_action" value="domain_query_logs" class="btn" style="background: #17a2b8;">🌐 清理域名查询日志</button>
