    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['alert_cooldown_minutes'])) {
        try {
            ...
            foreach ($settings_arr as $key => $value) {
                // 用 ON DUPLICATE KEY UPDATE 一次性搞定：存在就更新，不存在就插入
                // 比之前的 SELECT+INSERT/UPDATE 更安全（避免并发冲突和脏数据导致的 UNIQUE 冲突）
                $stmt = $conn->prepare("
                    INSERT INTO alert_settings (setting_name, setting_value, created_at, updated_at)
                    VALUES (?, ?, NOW(), NOW())
