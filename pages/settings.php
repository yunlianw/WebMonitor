<div class="section">
    ...
    <div style="background: #FFFFFF; border-radius: 16px; padding: 24px; border: 1px solid #F2F2F7; margin-bottom: 24px;">
        ...
        <div class="filter-tabs" style="margin-bottom: 16px;">
            ...
            <button type="button" onclick="switchLogTab('domain')" id="btn-log-domain" class="filter-tab">🌐 域名查询日志</button>
        </div>
        ...
        <div id="log-panel-domain" style="display: none;">
            ...
            <div style="overflow-x: auto; max-height: 400px; overflow-y: auto;">
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>查询接口</th>
                            <th style="text-align: center;">状态</th>
                            <th style="text-align: center;">到期天数</th>
                            <th>详情</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td style="white-space: nowrap; color: #86868B;"><?php echo date('m-d H:i', strtotime($log['query_time'])); ?></td>
                            <td><?php echo htmlspecialchars($log['domain']); ?></td>
                            <td>
                                <span class="type-label domain"><?php echo htmlspecialchars($log['api_used']); ?></span>
                            </td>
                            <td style="text-align: center;">
                                <?php if ($log['success']): ?>
                                    <span style="color: #34C759;">✓ 成功</span>
                            ...
                            <td style="text-align: center;">
                                <?php if ($log['days_remaining'] !== null): ?>
                                    <?php if ($log['days_remaining'] > 30): ?>
                                        <span style="color: #34C759;"><?php echo $log['days_remaining']; ?>天</span>
                                    <?php elseif ($log['days_remaining'] > 7): ?>
                                        <span style="color: #FF9500;"><?php echo $log['days_remaining']; ?>天</span>
                                    <?php else: ?>
                                        <span style="color: #FF3B30;"><?php echo $log['days_remaining']; ?>天</span>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <span style="color: #86868B;">-</span>
                                <?php endif; ?>
                            </td>
                            <td style="font-size: 0.8125rem; color: #86868B; max-width: 250px; word-break: break-all;">
                                <?php
                                $detailParts = [];
                                if ($log['expiry_date']) {
                ...
                <?php if (empty($domainLogs)): ?>
                    <p style="text-align: center; color: #86868B; padding: 32px;">暂无域名查询日志</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div style="background: #FFFFFF; border-radius: 16px; padding: 24px; border: 1px solid #F2F2F7; margin-bottom: 24px;">
        ...
        <table class="table" style="margin-bottom: 20px;">
            ...
            <tbody>
                ...
                <tr>
                    <td>🌐 域名查询日志</td>
                    <td style="text-align: center; font-weight: 500;"><?php echo $stats['domain_query_logs']; ?> 条</td>
                    <td style="text-align: center;"><?php echo $settings['history_retention_days'] ?? 30; ?>天</td>
                </tr>
            </tbody>
        </table>
        ...
        <form method="POST" onsubmit="return confirm('确定要清理数据吗？此操作不可恢复！');">
            ...
            <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                ...
                <button type="submit" name="clean_action" value="domain_query_logs" class="btn" style="background: #5AC8FA;">🌐 清理域名查询日志</button>
