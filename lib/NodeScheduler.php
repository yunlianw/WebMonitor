...
class NodeScheduler {
    ...
    private function processResults($sites, $results, $nodeId) {
        ...
        foreach ($results as $result) {
            ...
            // V4.2修复：check_http=0 / check_ssl=0 的网站，对应检测跳过，绝不触发告警
            $httpSkipped = !empty($result['http_skipped']) || $status === 'skipped';
            $sslSkipped = !empty($result['ssl_skipped']);
            
            // HTTP告警（跳过HTTP检测的网站不进入告警判断）
            if (!$httpSkipped && $status === 'down') {
                $canAlert = true;
            ...
            } elseif (!$httpSkipped && $lastStatus === 'down' && $status === 'up') {
                $alerts['http_up'][] = ['id' => $site['id'], 'name' => $site['name'], 'url' => $site['url']];
            }
            
            // SSL告警（V3.9: 包含已过期证书 $sslDays <= 0 也需告警；V4.2: 跳过SSL检测的网站不告警）
    ...
    private function logResult($site, $result, $nodeId) {
        ...
        $sslWarning = (int)($this->settings['ssl_warning_days'] ?? 60);
        if ($sslDays !== null) {
            if ($sslDays <= 0) $sslStatus = 'expired';          // 已过期
            elseif ($sslDays <= 7) $sslStatus = 'expired';      // 即将过期
            elseif ($sslDays <= $sslWarning) $sslStatus = 'warning'; // 在告警范围内
            else $sslStatus = 'valid';                          // 正常
        }
        
        // V4.2: check_type 根据实际执行的检测项记录（http/ssl/both）
        $httpSkipped = !empty($result['http_skipped']) || ($result['http_status'] ?? '') === 'skipped';
        $sslSkipped = !empty($result['ssl_skipped']);
        $checkType = 'both';
        ...
        // V3.4: 记录http_message（跨域跳转诊断信息，2026-08-16）
        $stmt = $this->conn->prepare("
            INSERT INTO monitor_logs (website_id, node_id, check_type, http_status, http_code, http_message, response_time, ssl_status, ssl_days, checked_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->execute([
            $site['id'],
