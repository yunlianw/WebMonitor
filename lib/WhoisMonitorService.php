...
class WhoisMonitorService {
    ...
    public static function checkBatch($conn, $websites) {
        ...
        foreach ($websites as $website) {
            ...
            // 记录查询日志
            self::logQuery($conn, $siteId, $domain, $whoisResult, $queryDuration);
     ...
     * @param array $result WHOIS查询结果
     * @param float $duration 查询耗时（毫秒）
     */
    private static function logQuery($conn, $websiteId, $domain, $result, $duration) {
        try {
            $stmt = $conn->prepare("
                INSERT INTO domain_query_logs 
                (website_id, domain, query_time, api_used, success, error_message, expiry_date, days_remaining, raw_response) 
                VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?)
            ");
            
            $success = ($result['status'] === 'success') ? 1 : 0;
            $errorMsg = $result['message'] ?? null;
            $expiryDate = $result['expire_date'] ?? null;
            $daysRemaining = $result['days'] ?? null;
            $apiUsed = $result['source'] ?? 'unknown';
            
            // 构造原始响应信息（包含查询耗时）
            $rawResponse = json_encode([
                'duration_ms' => $duration,
                'raw_date' => $result['raw_date'] ?? null,
            ...
            error_log("域名查询日志写入失败: " . $e->getMessage());
        }
    }
     ...
     */
    public static function checkAlerts($conn, $results, $force = false) {
        // 获取告警配置
        ...
        foreach ($results as $result) {
            ...
            // 检查是否达到告警阈值
            // 1. 即将过期：剩余天数在 (0, warningDays] 范围内
            // 2. 已经过期：$days <= 0 也需要告警（修复 #52qixia.com 过期40天未告警问题）
            if (($days > 0 && $days <= $warningDays) || $days <= 0) {
                // 检查冷却时间（force=true 时无视冷却）
                if ($force || self::canAlert($conn, $siteId, $cooldownHours)) {
                    $alerts[] = $result;
     ...
     * 获取需要检测的网站列表
     */
    public static function getWebsitesToCheck($conn, $intervalHours = 24, $force = false) {
        if ($force) {
            // 强制模式：返回所有启用了 whois 监控的网站，忽略时间间隔
            $sql = "
                SELECT id, name, url, host, last_whois_check
                FROM websites
                WHERE enabled = 1 AND check_whois = 1
