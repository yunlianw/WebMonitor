...
class WhoisChecker {
     ...
     */
    // V1.1: 主域名结果缓存（避免 *.example.com 这类子域名重复查主域名）
    private static $registrableCache = [];
    ...
    public static function check($domain) {
        ...
        // V1.1: 子域名/三级/四级域名查不到时，降级查主域名（如 302.73sm.net → 73sm.net）
        $registrable = self::getRegistrableDomain($domain);
        if ($registrable && $registrable !== $domain) {
            if (isset(self::$registrableCache[$registrable])) {
                $cached = self::$registrableCache[$registrable];
                if ($cached['status'] === 'success') {
                    return [
                        'status' => 'success',
                        'days' => $cached['days'],
                        'expire_date' => $cached['expire_date'],
                        'source' => $cached['source'] . ' (主域名)',
                        'raw_date' => $cached['raw_date'] ?? null,
            ...
            $r = self::checkWhois4cn($registrable);
            if ($r['status'] !== 'success') $r = self::checkTencentWhois($registrable);
            if ($r['status'] !== 'success') $r = self::checkRdap($registrable);
            
            self::$registrableCache[$registrable] = $r;
    ...
    /**
     * V1.1: 提取主域名（注册的二级域名）
     * 规则：取最后两段（example.com / example.cn / example.org 等）
     *      例：302.73sm.net → 73sm.net
     *          a.b.example.co.uk → example.co.uk
     *      已知多段后缀（co.uk/com.cn 等）会保留
     */
    public static function getRegistrableDomain($domain) {
        ...
        $multiPartSuffixes = [
            ...
            'com.sg', 'com.my', 'com.ph', 'com.tr',
            'org.uk', 'ac.uk', 'gov.uk'
        ];
        
        foreach ($multiPartSuffixes as $suffix) {
            if (substr($domain, -strlen('.' . $suffix)) === '.' . $suffix) {
                $parts = explode('.', $domain);
                // 保留 suffix 之前的最后一段 + suffix
                if (count($parts) >= 3) {
                    return $parts[count($parts) - 3] . '.' . $suffix;
                }
                return $domain;
            }
        }
